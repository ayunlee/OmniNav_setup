#!/usr/bin/env python3
"""
OmniNav inference using iPhone panorama 3-view data
Based on evaluate_agent from infer_r2r_rxr,
but replaces simulator image input with iPhone data reading

Supports panorama 3-view data structure:
  rgb/frame_XXXX/frame_XXXX_front.jpg
  rgb/frame_XXXX/frame_XXXX_left.jpg
  rgb/frame_XXXX/frame_XXXX_right.jpg
"""
...
DATA_ID="Test18" && \
OUTPUT_DIR="result_panorama_${DATA_ID}_$(date +%Y%m%d_%H%M%S)" && \
LOG_FILE="/tmp/panorama_inference_${DATA_ID}.log" && \
docker run --gpus all --rm \
  -v /home/aprl_msi/OmniNav_20260115_giseopkim:/workspace/OmniNav \
  --shm-size=8gb \
  -w /workspace/OmniNav/infer_r2r_rxr \
  omninav:aarch64 \
  python3 run_infer_iphone_panorama.py \
...
echo "PID: $! | 출력: $OUTPUT_DIR | 로그: $LOG_FILE"
"""
import os
import sys
...
def get_rgb_images(rgb_dir):
    """Get RGB frame folder list (sorted)
    
    Returns list of frame folder paths (not individual image files)
    Each folder contains front, left, right images
    """
    rgb_path = Path(rgb_dir)
    # Find frame_XXXX folders
...
def create_fake_observations(frame_idx, frame_folder_path, instruction, pose):
    """Create dictionary mimicking simulator observations
    
    Panorama 3-view data: Load front, left, right images separately
    """
    frame_folder = Path(frame_folder_path)
    frame_name = frame_folder.name  # e.g., "frame_0000"
    
    # Construct paths for each view
    ...
    observations = {
        ...
        'left': left_rgb,
        'right': right_rgb,
        'rgb': front_rgb,  # Main RGB uses front
...
def evaluate_iphone_data(data_dir, model_path, result_path, max_frames=0):
    ...
    Args:
        data_dir: iPhone data directory (contains instruction.txt, rgb/)
                  odometry.csv is optional
    ...
    for i in trange(num_frames, desc=f"Processing {episode_id}"):
        frame_folder_path = image_files[i]  # This is now a folder path
        frame_name = Path(frame_folder_path).name  # e.g., "frame_0000"
        frame_idx = int(frame_name.split('_')[1])  # Extract frame index from folder name
        
        # Find pose for this frame from odometry (use default if no odometry)
        pose = {
        ...
        # Create observations
        obs = create_fake_observations(frame_idx, frame_folder_path, instruction, pose)
        ...
        if 'arrive_pred' in action and 'action' in action and 'recover_angle' in action:
            ...
            # Simple log output (all waypoints)
            wp = waypoints[0]
            dthetas = []
            for i in range(len(recover_angles)):
                dthetas.append(np.degrees(recover_angles[i]))
            
            dthetas_str = ", ".join([f"{dt:.2f}" for dt in dthetas])
            print(f"  -> arrive={arrive}, dthetas=[{dthetas_str}], wp[0]=({wp[0]:.4f}, {wp[1]:.4f})")
