#!/usr/bin/env python3
"""
SocialACT Dataset Evaluation using OmniNav — LOW-LEVEL INSTRUCTIONS ONLY
...
This script runs ONLY the Low-level instruction round so that it can be run
in parallel with `run_infer_socialact_high.py`.

Usage:
    python run_infer_socialact_low.py --model-path ../OmniNav --dataset-dir ../SocialACT
"""
...
def parse_instruction_xlsx(xlsx_path):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        num_subs_val = row[1]
        if num_subs_val is None:
            continue
        num_subs = int(num_subs_val)
        
        sub_missions = []
        for i in range(num_subs):
            start_time_col = 2 + i * 2
            instruction_col = 3 + i * 2
...
class SocialACTEvaluator:
    ...
    def __init__(self, model_path, dataset_dir):
        ...
        self._temp_agent_path = tempfile.mkdtemp(prefix="omninav_socialact_low_")

        print("=" * 60)
        print("[SocialACT-LOW] Loading OmniNav model (one-time) ...")
        print("=" * 60)
        self.agent = Waypoint_Agent(model_path, self._temp_agent_path, require_map=False)
        self.agent.reset()
        print("[SocialACT-LOW] Model loaded successfully.\n")
    ...
    def compute_sub_mission_ranges(sub_missions, last_frame_idx):
        ...
        Rules (from dataset design):
          ...
          - end_frame   = int(next_start_time * 2) - 1  for non-last sub-missions
        ...
        for i, (start_time, instruction) in enumerate(sub_missions):
            ...
            if i + 1 < n:
                ...
                end_frame = int(next_start_time * 2) - 1
            else:
    ...
    def evaluate_all(self, instruction_xlsx, output_dir):
        ...
        print(f"\n[SocialACT-LOW] Found {len(mission_dirs)} mission folders")
        print(f"[SocialACT-LOW] Instructions loaded for: {sorted(instructions.keys())}")
        print(f"[SocialACT-LOW] Output directory: {os.path.abspath(output_dir)}\n")
        ...
        print(f"\n[SocialACT-LOW] All missions evaluated.  Results in: {os.path.abspath(output_dir)}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate SocialACT dataset with OmniNav (Low-level instructions ONLY)')

    parser.add_argument("--model-path", type=str, default="../models/chongchongjj/OmniNav",
    ...
    dataset_dir = (args.dataset_dir
                   ...
                   else os.path.normpath(os.path.join(script_dir, "..", "SocialACT_Low_Test_Ayun")))
    output_base = (args.output_dir
                   if args.output_dir is not None
                   else os.path.normpath(os.path.join(script_dir, "..", "SocialACT_Low_Test_Ayun_Results")))

    low_xlsx = os.path.join(dataset_dir, "Instruction_10missions.xlsx")

    # Validate paths
    for p, label in [(dataset_dir, "Dataset dir"), (low_xlsx, "Low-level xlsx")]:
    ...
    # ---------- Low-level instructions ONLY ----------
    print("\n" + "=" * 80)
    print("  LOW-LEVEL INSTRUCTIONS ONLY")
    print("=" * 80)
    ...
    # ---------- Summary ----------
    print("\n" + "=" * 80)
    print("  LOW-LEVEL EVALUATION COMPLETE")
